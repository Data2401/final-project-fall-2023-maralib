...
- runs_scored
- sacrifice_flies: when a batter hits a fly-ball out that "sacrifices" themselves to bring in a baserunner to score a run
...
Because of what seems to be a slight positive relationship between the variables, I decided to add a regression line to the graph, which tries to predict the value of y for any given value of x. The formula for the regression line comes out to be y = 28,379X + 138,601. Although correlation doesn't mean causation, by using this regression line one would expect that for every win that a team achieves, their home attendance would increase by 28,379.
...
An interesting relationship I found was the relationship between

```{r}
Astros_data %>% ggplot(aes(x=sacrifice_flies, y=runs_scored)) +
  geom_jitter(col = "#002D62")

##cor.test(Astros_data$sacrifice_flies, Astros_data$runs_scored)
```
